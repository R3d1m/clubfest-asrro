...
import { Share2, CheckCircle2, Award, Sparkles } from 'lucide-react';
...
export const HeroSummary: React.FC<HeroSummaryProps> = ({
  ...
  return (
    <div className="w-full h-full flex flex-col justify-between animate-bounce-in overflow-hidden">
      {/* Hero Badge Card */}
      <div className="pop-box w-full flex-1 p-4 bg-[#FFFBEB] border-3.5 border-[#1E232A] text-center flex flex-col justify-between gap-3 shadow-pop-lg overflow-hidden">
        {/* Top Dept Emblem */}
        <div className="flex flex-col items-center flex-shrink-0">
          <div 
            className="w-16 h-16 rounded-3xl border-3 border-[#1E232A] shadow-pop flex flex-col items-center justify-center animate-bounce mb-1.5"
            style={{ backgroundColor: student.themeColor }}
          >
            <Award className="w-8 h-8 text-[#1E232A]" />
            <span className="font-black text-xs text-[#1E232A] leading-none">{student.deptAbbr}</span>
          </div>

          <div className="inline-flex items-center space-x-1 px-3 py-0.5 bg-[#D4F8F0] border border-[#1E232A] rounded-full text-xs font-black text-[#00897B] font-bangla mb-1">
            <CheckCircle2 className="w-3.5 h-3.5" />
            <span>সফলভাবে সম্পন্ন হয়েছে!</span>
          </div>

          <h2 className="text-xl font-black font-bangla text-[#1E232A]">
        ...
        <div className="p-3 bg-white rounded-2xl border-2.5 border-[#1E232A] shadow-pop-sm space-y-2 text-left font-bangla flex-1 flex flex-col justify-center">
          <div className="flex items-center justify-between border-b border-dashed border-gray-200 pb-1.5 text-xs sm:text-sm">
        ...
        <div className="w-full flex-shrink-0">
          <button
            onClick={handleShare}
            className="pop-btn w-full py-3.5 bg-[#4ECDC4] text-[#1E232A] font-black text-base font-bangla flex items-center justify-center space-x-2 shadow-pop cursor-pointer"
          >
            <Share2 className="w-4 h-4 text-[#1E232A]" />
